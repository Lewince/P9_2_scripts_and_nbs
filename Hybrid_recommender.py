...
class CBF_Recommender():
    ...
    def __init__(self, initial_clicks_db, user_indexed_clicks_db, article_features, threshold = 5):
    ...
    def article_selection(self, user_id, top_n=50, verbose=False):
        ...
        # Use content based principle if user has had at least {threshold} interactions
        if user_profile.shape[0] > self.threshold : 
            ...
            output = [i[0] for i in selection[top_n:]]
        # Else provide most popular items : 
        else : 
            already_consumed = self.initial_clicks_db[self.initial_clicks_db.loc[:,'user_id'] == user_id]
            already_consumed = already_consumed.loc[:,'click_article_id']
            popularity_df = clicks_df.groupby('click_article_id')['click_country'].size().sort_values(ascending=False).reset_index()
            recommendations_df = popularity_df[~popularity_df['click_article_id'].isin(already_consumed)] \
                               .head(top_n)
            output = recommendations_df.click_article_id.tolist()
        return output
